...
export const useProgress = () => {
  ...
  const updateProgress = useMutation({
    mutationFn: async ({ contentType, contentId, progressPercentage }: { contentType: ContentType; contentId: number; progressPercentage?: number }) => {
      if (!user?.id) throw new Error('No user logged in');
      ...
      const { data, error } = await supabase
        ...
        .upsert({
          ...
          content_type: contentType,
          content_id: contentId,
          progress_percentage: progressPercentage || current?.progress_percentage || 0,
  ...
  const getProgress = (contentType: ContentType, contentId: number) => {
